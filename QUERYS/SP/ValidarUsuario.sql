...
BEGIN	
	...
	IF @ValidarIntegracion = 1 AND ISNULL(@CodigoIntegracion,'') <> '' BEGIN
		...
		SELECT ID_USUARIO,RTRIM(NOMBRE) AS NOMBRE,RTRIM(CONTRASEÑA) AS CONTRASEÑA,RTRIM(PERFIL) AS PERFIL,
		CAST(ID_EMPRESA AS VARCHAR(36)) AS ID_EMPRESA,RTRIM(CODIGO_INTEGRACION) AS CODIGO_INTEGRACION,
	...
	ELSE BEGIN
		...
		@HashKey AS HashKey,RTRIM(EMPRESA.MAIL) AS MAIL FROM USUARIOS
