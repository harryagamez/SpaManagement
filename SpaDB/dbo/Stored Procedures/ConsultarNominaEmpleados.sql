...
BEGIN
	...
	INNER JOIN #TempServicios_Empleados ServiciosEmpleados ON ServiciosEmpleados.Id_Empleado = NominaEmpleados.Id_Empleado AND ServiciosEmpleados.Id_Empresa = NominaEmpleados.Id_Empresa	
	...
	IF(@TipoNomina = 'POR_SERVICIOS') BEGIN
		SELECT 
			Id_Empresa, Id_Empleado, Nombres, Apellidos, Servicios, Prestamos, Salario, (Servicios * Salario) AS Total_Aplicado, ((Servicios * Salario) - ISNULL(Prestamos, 0)) AS Total_Pagar, RTRIM(Tipo_Nomina) AS Tipo_Nomina
		FROM #TempNomina_Empleados
	END
	ELSE BEGIN
		SELECT 
			Id_Empresa, Id_Empleado, Nombres, Apellidos, Servicios, Prestamos, Salario, (Salario) AS Total_Aplicado, (Salario - ISNULL(Prestamos, 0)) AS Total_Pagar, RTRIM(Tipo_Nomina) AS Tipo_Nomina
		FROM #TempNomina_Empleados		
	END
