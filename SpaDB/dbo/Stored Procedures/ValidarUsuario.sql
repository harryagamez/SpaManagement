...
BEGIN	
	...
	IF @ValidarIntegracion = 1 AND ISNULL(@CodigoIntegracion,'') <> '' BEGIN
		...
		SELECT 
			ID_USUARIO,RTRIM(USUARIOS.NOMBRE) AS NOMBRE,RTRIM(CONTRASENIA) AS CONTRASEÑA,RTRIM(PERFIL) AS PERFIL,
	...
	ELSE BEGIN
		...
		SELECT 
			...
			VERIFICADO,@HashKey AS HashKey,RTRIM(EMPRESA.MAIL) AS MAIL,EMPRESA.NOMBRE AS NOMBRE_EMPRESA 
		FROM USUARIOS 
		INNER JOIN EMPRESA 
