...
BEGIN
	...
	;WITH EmpleadoCitas AS (
		...
		AND ID_EMPLEADO = @IdEmpleado AND ESTADO = 'PROGRAMADA' OR ESTADO = 'CONFIRMADA' AND ID_AGENDA <> @IdAgenda
	)
	...
	;WITH ClienteCitas AS (
		...
		FROM AGENDA 
		WHERE CONVERT(CHAR(10),FECHA_INICIO,120) = @FechaAgendaCita 
		AND ID_CLIENTE = @IdCliente AND ESTADO = 'PROGRAMADA' OR ESTADO = 'CONFIRMADA' AND ID_AGENDA <> @IdAgenda
	)
