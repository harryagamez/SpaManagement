	...
	BEGIN TRY
		...
		IF @Distribucion = 1 BEGIN
			...
			MERGE CAJA_MENOR AS TARGET
				...
				ON  TARGET.ID_EMPRESA = SOURCE.Id_Empresa AND TARGET.AÑO = CAST(YEAR(GETDATE()) AS INT) AND TARGET.MES = CAST(MONTH(GETDATE()) AS INT) AND CONVERT(date, TARGET.DIA) = CONVERT(date, GETDATE())
				WHEN MATCHED THEN
					UPDATE 
				...
				WHEN NOT MATCHED THEN
					INSERT (AÑO, MES, DIA, SALDO_INICIAL, ACUMULADO, FECHA_REGISTRO, FECHA_MODIFICACION, ID_EMPRESA)
		...
		IF @Distribucion = 2 BEGIN
			...
			MERGE CAJA_MENOR AS TARGET
				...
				ON  TARGET.ID_EMPRESA = SOURCE.Id_Empresa AND TARGET.AÑO = CAST(YEAR(GETDATE()) AS INT) AND TARGET.MES = CAST(MONTH(GETDATE()) AS INT)
				WHEN MATCHED THEN
					UPDATE 
						SET TARGET.ACUMULADO = (TARGET.ACUMULADO + SOURCE.Saldo_Inicial), TARGET.SALDO_INICIAL = (TARGET.SALDO_INICIAL + SOURCE.Saldo_Inicial), TARGET.FECHA_MODIFICACION = GETDATE()
				WHEN NOT MATCHED THEN
					INSERT (AÑO, MES, SALDO_INICIAL, ACUMULADO, FECHA_REGISTRO, FECHA_MODIFICACION, ID_EMPRESA)
