		...
		BEGIN TRANSACTION Tn_ReemplazarCajaMenor		
			...
			IF @Distribucion = 2 BEGIN
				...
				MERGE CAJA_MENOR AS TARGET
					...
					WHEN MATCHED THEN
						UPDATE 
							SET TARGET.ACUMULADO = (SOURCE.Acumulado + SOURCE.Saldo_Inicial), TARGET.SALDO_INICIAL = SOURCE.Saldo_Inicial, TARGET.FECHA_MODIFICACION = GETDATE()
					WHEN NOT MATCHED THEN
						INSERT (ANIO, MES, SALDO_INICIAL, ACUMULADO, FECHA_REGISTRO, FECHA_MODIFICACION, ID_EMPRESA)
						VALUES (YEAR(GETDATE()), MONTH(GETDATE()), SOURCE.Saldo_Inicial, (SOURCE.Acumulado + SOURCE.Saldo_Inicial), GETDATE(), GETDATE(), SOURCE.Id_Empresa);
			END
